/* credit : BangHasan & Aghisna serta Bagus Setiawan, dam seluruh anggota grup telegrambotindonesia
yang sudah banyak membantu saya yang tidak bisa saya sebutkan satu persatu"

Bot ini hanya jiplakan jadi jangan ngarep banyak ya wkwkwkwkw*/



const token = 'your_bot token';

const bot = new lumpia.init(token, { log_id: admin_id, prefix_command: "!/." });

var admin_id = 12345 //your tg.id get it from @strukturbot
...
function cariInfoSholat(cari) {
  ...
  if (respon.length == 0) {
    return "Maaf, Pencarian Tidak Ditemukan, Silahkan masukkan Query hanya Kotamadya dan Kabupaten saja";
  }
  if (respon.length > 1) {
...
function setWebhook() {
  let url = 'https://script.google.com/macros/s/AKfycbxg7A9I88AEsVj8NrxGrnPolTpA_vUWpasXKHMHMQrOHrPH0Rix3lQyc1Hqw4sVRe2UIA/exec';
